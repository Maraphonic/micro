...
import com.d.t.config.KafkaProperties;
...
class MicroKafkaResourceIT {
    ...
    void setup() {
        ...
        MicroKafkaResource kafkaResource = new MicroKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/micro-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/micro-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
